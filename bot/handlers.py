...
class HelpHandler(BaseHandler):
    ...
    async def show_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_text = (
            "📚 <b>Помощь по боту Таро</b>\n\n"
            "🔮 <b>Основные функции:</b>\n"
            "1. 🃏 <b>Расклад</b> — получите ответ на ваш вопрос с помощью карт Таро\n"
            "2. 💎 <b>Подписка</b> — неограниченные расклады за фиксированную плату\n"
            "3. 📜 <b>Значения карт</b> — полная база значений всех карт Таро\n"
            "4. 📞 <b>Консультация</b> — персональный разбор от профессионального таролога\n\n"
            "❓ <b>Как работать с ботом:</b>\n"
            "- Выберите действие в главном меню\n"
            "- Следуйте инструкциям бота\n"
            "- Для отмены текущего действия используйте /cancel\n\n"
            f"📩 <b>Поддержка:</b> @{Config.ADMIN_USERNAME}\n"
    ...
    async def show_subscriptions(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            text = (
                ...
                "1. 💎 Месячная подписка - 349₽\n"
                "2. 🛒 Разовые пакеты попыток"
            )
    ...
    async def begin_reading(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await context.bot.send_message(
            ...
            text="🔮 *Задайте ваш вопрос*\n\n"
                 "Пример: «Что меня ждет в любви в ближайший месяц?»",
            parse_mode="Markdown"
        )
        return QUESTION
...
class ReferralHandler(BaseHandler):
    ...
    async def invite(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text = (
            ...
            f"Получено бонусов: <b>{bonuses}</b>\n\n"
            "За каждого друга — +1 попытка! "
            "Нажмите «Поделиться», чтобы быстро отправить ссылку в чаты и друзьям."
        )
        await context.bot.send_message(
